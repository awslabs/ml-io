...
public:
    ...
    explicit intrusive_ptr(T *ptr, bool inc_ref = true) noexcept : ptr_{ptr}
    {
        if (inc_ref) {
            inc_ref_count();
        }
    ...
    template<typename U>
    // NOLINTNEXTLINE(google-explicit-constructor)
    intrusive_ptr(intrusive_ptr<U> const &other) noexcept : ptr_{static_cast<T *>(other.ptr_)}
    {
...
public:
    ...
    template<typename U>
    intrusive_ptr &operator=(intrusive_ptr<U> const &other) noexcept
    {
        static_assert(std::is_convertible<U *, T *>::value, "U must be convertible to T.");
...
template<typename T>
MLIO_API
inline constexpr bool operator==(intrusive_ptr<T> const &lhs, intrusive_ptr<T> const &rhs)
{
...
template<typename T>
MLIO_API
inline constexpr bool operator==(intrusive_ptr<T> const &lhs, std::nullptr_t)
{
...
template<typename T>
MLIO_API
inline constexpr bool operator==(std::nullptr_t, intrusive_ptr<T> const &rhs)
{
...
template<typename T>
MLIO_API
inline intrusive_ptr<T> wrap_intrusive(T *t, bool inc_ref = true)
{
...
struct MLIO_API hash<mlio::intrusive_ptr<T>> {
    inline size_t operator()(mlio::intrusive_ptr<T> const &ptr) const noexcept
    {
