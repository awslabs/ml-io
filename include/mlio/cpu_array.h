...
private:
    explicit cpu_array(data_type dt, Container const &cont)
        : data_type_{dt}, container_{cont}
    {}

    explicit cpu_array(data_type dt, Container &&cont)
...
template<typename Container>
std::unique_ptr<device_array>
cpu_array<Container>::clone() const
{
...
struct cpu_array_access {
    ...
    {
        auto *ptr =
            new cpu_array<Container>{dt, std::forward<Container>(cont)};
