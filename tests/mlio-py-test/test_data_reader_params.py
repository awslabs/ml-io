...
def test_data_reader_params_members():
    ...
    assert rdr_prm.num_instances_to_read == None
    assert rdr_prm.shard_index == 0
    assert rdr_prm.num_shards == 0
    assert rdr_prm.shuffle_instances == False
    assert rdr_prm.shuffle_window == 0
    assert rdr_prm.shuffle_seed == None
    assert rdr_prm.reshuffle_each_epoch == False
    assert rdr_prm.subsample_ratio == None
