...
protected:
    ...
    std::vector<std::size_t> image_dimensions_std_{static_cast<std::size_t>(ret_img_3channel_),
                                                   static_cast<std::size_t>(ret_img_height_),
                                                   static_cast<std::size_t>(ret_img_width_)};
    std::vector<std::size_t> image_dimensions_1_ch_{static_cast<std::size_t>(ret_img_1channel_),
                                                    static_cast<std::size_t>(ret_img_height_),
                                                    static_cast<std::size_t>(ret_img_width_)};
    ...
    std::string unsupported_channel_exception_{
        "The specified image dimensions have an unsupported number of channels (5)."};
    ...
    std::string invalid_image_dimensions_exception_{
        "The input image dimensions (rows: 166, cols: 190) are smaller than the output image dimensions (rows: 500, cols: 500) for the image #0 in the data store"};

public:
    cv::Mat get_expected_image(const std::string &img_path,
                               std::size_t resize = 0,
...
TEST_F(Test_image_reader, test_jpg_3_channel_last_batch_dropped)
{
    ...
    for (auto i = 0; i < 2; i++) {
        ...
        while ((exm = reader->read_example()) != nullptr) {
            ...
            cv::Mat output;
            std::size_t resize = 0;
            cv::bitwise_xor(get_expected_image(img0_path_jpg_, resize), img_0, output);
...
TEST_F(Test_image_reader, test_jpg_1_channel)
{
    ...
    cv::Mat img_1{ret_img_height_,
                  ret_img_width_,
                  CV_8UC1,
                  image_buffer.data() + (ret_img_height_ * ret_img_width_ * ret_img_1channel_)};

    ASSERT_NE(image_buffer.data(), nullptr);
    cv::Mat output;
    cv::bitwise_xor(
        get_expected_image(img0_path_jpg_, resize, cv::IMREAD_GRAYSCALE), img_0, output);
...
TEST_F(Test_image_reader, test_invalid_image_dimensions_parameter)
{
    ...
    try {
        ...
        ASSERT_EQ(std::string(ex.what()).find(invalid_image_dimensions_exception_), 0);
    }
    ...
    try {
        ...
        ASSERT_EQ(std::string(ex.what()), missing_image_dimensions_prm_exception_);
    }
}
...
TEST_F(Test_image_reader, test_bad_example_handling_skip)
{
    ...
    for (auto i = 0; i < 2; i++) {
        ...
        while ((exm = reader->read_example()) != nullptr) {
            ...
            ASSERT_NE(image_buffer.data(), nullptr);
            cv::Mat output;
            cv::Mat expected_image = cv::imread(img2_path_jpg_, cv::IMREAD_COLOR);
