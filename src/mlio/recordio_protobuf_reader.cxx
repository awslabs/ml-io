...
public:
    explicit
    decoder_state(recordio_protobuf_reader const &reader, std::size_t batch_size);
...
private:
    ...
    void
    init_tensor(feature_desc const &desc, std::size_t batch_size);

    void
    init_coo_tensor_builder(feature_desc const &desc, std::size_t batch_size);
...
public:
    explicit
    decoder(recordio_protobuf_reader const &reader, decoder_state &state)
...
infer_schema(instance const &ins)
{
    ...
    for (auto &pr : proto_msg->label()) {
        ...
        descs.emplace_back(make_feature_desc(ins, "label_" + pr.first, pr.second));
    }
    for (auto &pr : proto_msg->features()) {
        descs.emplace_back(make_feature_desc(ins, pr.first, pr.second));
    }
...
feature_desc
recordio_protobuf_reader::
make_feature_desc(instance const &ins, std::string const &name, Value const &value)
{
    switch (value.value_case()) {
    case Value::ValueCase::kFloat32Tensor:
        return make_feature_desc<data_type::float32>(ins, name, value.float32_tensor());

    case Value::ValueCase::kFloat64Tensor:
        return make_feature_desc<data_type::float64>(ins, name, value.float64_tensor());

    case Value::ValueCase::kInt32Tensor:
        return make_feature_desc<data_type::sint32> (ins, name, value.int32_tensor());
    ...
    throw schema_error{fmt::format(
        ...
        ins.get_data_store(), ins.index() + 1 , name)};
}
                  ...
                  ProtobufTensor const &tsr)
{
    ...
    return feature_desc_builder{name, dt, std::move(shape)}
        .with_sparsity(is_sparse).build();
}
...
parse_proto(instance const &ins)
{
    bool parsed = detail::proto_msg_.ParseFromArray(
        ins.bits().data(),
        static_cast<int>(ins.bits().size()));

    return parsed ? &detail::proto_msg_ : nullptr;
}

recordio_protobuf_reader::decoder_state::
decoder_state(recordio_protobuf_reader const &reader, std::size_t batch_size)
{
...
init_state(schema const &shm, std::size_t batch_size)
{
    tensors.reserve(shm.descriptors().size());

    coo_tensor_builders.reserve(shm.descriptors().size());

    for (feature_desc const &desc : shm.descriptors()) {
        if (desc.sparse()) {
            init_coo_tensor_builder(desc, batch_size);
        } else {
            init_tensor(desc, batch_size);
        }
    }
}
...
init_tensor(feature_desc const &desc, std::size_t batch_size)
{
    std::size_t data_size = batch_size * as_size(desc.strides()[0]);

    auto arr = make_cpu_array(desc.dtype(), data_size);

    size_vector shape = desc.shape();
    shape[0] = batch_size;
...
init_coo_tensor_builder(feature_desc const &desc, std::size_t batch_size)
{
    auto bld = make_coo_tensor_builder(desc, batch_size);
...
bool
recordio_protobuf_reader::decoder::
decode(std::size_t row_idx, instance const &ins)
{
    row_idx_ = row_idx;
    ...
    for (auto &pr : proto_msg->label()) {
        if (!decode_feature("label_" + pr.first, pr.second)) {
            return false;
        }
    ...
    for (auto &pr : proto_msg->features()) {
        if (!decode_feature(pr.first, pr.second)) {
            return false;
        }
    ...
    if (state_->bbh != bad_batch_handling::skip) {
        auto msg = fmt::format(
            ...
            instance_->get_data_store(), instance_->index() + 1,
            num_features_read, reader_->schema_->descriptors().size());
...
decode_feature(std::string const &name, Value const &value)
{
    stdx::optional<std::size_t> idx = reader_->schema_->get_index(name);
    if (idx == stdx::nullopt) {
        if (state_->bbh != bad_batch_handling::skip) {
            auto msg = fmt::format(
                "The record {1:n} in the data store {0} has an unknown "
                "feature named '{2}'.",
                instance_->get_data_store(), instance_->index() + 1, name);
...
decode_feature(ProtobufTensor const &tsr)
{
    ...
    if (is_sparse(tsr) != ftr_dsc_->sparse()) {
        if (state_->bbh != bad_batch_handling::skip) {
            char const *ft;
            if (ftr_dsc_->sparse()) {
                ft = "The feature '{2}' of the record {1:n} in the data store "
                     "{0} is sparse, while the expected storage type is dense.";
            } else {
                ft = "The feature '{2}' of the record {1:n} in the data store "
                     "{0} is dense, while the expected storage type is sparse.";
            }
            auto msg = fmt::format(ft, instance_->get_data_store(),
                                   instance_->index() + 1, ftr_dsc_->name());
    ...
    if (!shape_equals(tsr)) {
        if (state_->bbh != bad_batch_handling::skip) {
            ...
            size_vector const &shape = ftr_dsc_->shape();
...
{
    // The stride of the batch dimension.
    std::ptrdiff_t num_values = ftr_dsc_->strides()[0];
    ...
    if (tsr.keys_size() != tsr.values_size()) {
        if (state_->bbh != bad_batch_handling::skip) {
            auto msg = fmt::format(
                ...
                instance_->get_data_store(), instance_->index() + 1,
                ftr_dsc_->name(), tsr.keys_size(), tsr.values_size());
